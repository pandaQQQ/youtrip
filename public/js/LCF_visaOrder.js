...
/*=======================减成人人数======================*/
var antiFee = document.getElementsByClassName("antiFee")[0];
var insurance = document.getElementsByClassName("insurance")[0];
var checkBoxfee = document.getElementsByName("checkboxfee")[0];
var checks = document.getElementsByName("checkbox");

var adultnum;
var peopleNumber = document.getElementById("peopleNumber")//签证人数
var peopleFree = document.getElementById("peopleFree")//保险费人数
var peopleanti = document.getElementById("peopleanti")//拒签费人数
var visafree = document.getElementById("visafree")//签证费用
var ktfree = document.getElementById("ktfree")//快递费用
var free = document.getElementById("free");//意外保险费用
var antifree = document.getElementById("antifree");//拒签费用
var sumFree = document.getElementById("sumFree");//保险费用

/*=======================减成人人数======================*/
function  reduceAdult(){
    var numValue1 =document.getElementById("numValue1");
    adultnum =parseInt(numValue1.value)
    if(adultnum==1){
        numValue1.valueOf().value=1
        peopleNumber.innerText=1
        peopleFree.innerText=1
        peopleanti.innerText=1
        sumFree.innerText=parseInt(visafree.innerText)+parseInt(free.innerText)+parseInt(antifree.innerText)+parseInt(ktfree.innerText)
    }else{
        numValue1.valueOf().value=adultnum-1
        peopleNumber.innerText=adultnum-1
        peopleFree.innerText=adultnum-1
        peopleanti.innerText=adultnum-1
        sumFree.innerText=(parseInt(visafree.innerText)*(parseInt(adultnum-1)))+parseInt(free.innerText)*(parseInt(adultnum-1))+parseInt(antifree.innerText)*(parseInt(adultnum-1))+parseInt(ktfree.innerText)
    }
    if($(".addvisitor").length>1){
        $(".addvisitor").eq(adultnum-1).remove()
    }
}
/*=======================加成人人数======================*/
function addAdult(){
    ...
    peopleanti.innerText=(adultnum+1);
    sumFree.innerText=(parseInt(visafree.innerText)*(parseInt(adultnum+1)))+parseInt(free.innerText)*(parseInt(adultnum+1))+parseInt(antifree.innerText)*(parseInt(adultnum+1))+parseInt(ktfree.innerText)
    var numValue1 = parseInt(document.getElementById("numValue1").valueOf().value);
    var addvisitor= $("<div class='col-lg-5 col-md-5 col-sm-5 addvisitor'>"+
            "<div class='col-lg-2 col-md-2 col-sm-2'>出游人"+numValue1+"</div>"+
            "<div class='col-lg-10 col-md-10 col-sm-10'>"+
            "<div class='row visitor1'>"+
            "<div class='col-lg-4 col-sm-4 col-md-4'>姓&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名</div>"+
            "<div class='col-lg-8 col-sm-8 col-md-8 orderInput1'>"+
            "<input type='text' placeholder='请输入您的姓名'  name='pelName'>"+
            "</div>"+
            "</div>"+
            "<div class='row visitor1'>"+
            "<div class='col-lg-4 col-sm-4 col-md-4'>客户类型</div>"+
            "<div class='col-lg-8 col-sm-8 col-md-8 orderInput1'>"+
            "<select class='typeSelect'>"+
            "<option value=''>在职人员</option>"+
            "<option value=''>自由职业者</option>"+
            "<option value=''>退休人员</option>"+
            "<option value=''>在校学生</option>"+
            "<option value=''>学龄前儿童</option>"+
            "</select>"+
            "</div>"+
            "</div>"+
            // "<div class='row visitor1'>"+
            // "<div class='col-lg-4 col-sm-4 col-md-4'>出生日期</div>"+
            // "<div class='col-lg-8 col-sm-8 col-md-8 orderInput2'>"+
            // "<select class='sel_year' rel='2000'></select> 年"+
            // "<select class='sel_month' rel='2'></select> 月"+
            // "<select class='sel_day' rel='1'></select> 日"+
            // "</div>"+
            // "</div>"+
            "</div>"+
            "</div>");
    $(".addpeople").append(addvisitor)
}
...
function check(num){
    numSum =num
    if(checks[num].checked){
        for(var i=0;i<checks.length;i++){
            checks[i].checked = false;
        }
        checks[num].checked = true;
        insurance.style.display="block";
        var val= parseInt(checks[num].parentNode.previousSibling.previousSibling.childNodes[1].innerText);
        free.innerText=val
        console.log(val)
        if(checkBoxfee.checked){
            sumFree.innerText=(parseInt(visafree.innerText)*(parseInt(peopleNumber.innerText)))+parseInt(free.innerText)*(parseInt(peopleFree.innerText))+parseInt(antifree.innerText)*(parseInt(peopleanti.innerText))+parseInt(ktfree.innerText)
        }else{
            sumFree.innerText=(parseInt(visafree.innerText)*(parseInt(peopleNumber.innerText)))+parseInt(free.innerText)*(parseInt(peopleFree.innerText))+parseInt(ktfree.innerText)
        }
    }else {
        for (var i = 0; i < checks.length; i++) {
            checks[i].checked = false;
        ...
        if (checkBoxfee.checked) {
            sumFree.innerText = parseInt(visafree.innerText)*parseInt(peopleNumber.innerText)+parseInt(antifree.innerText)*parseInt(peopleanti.innerText) + parseInt(ktfree.innerText)
        }else{
            sumFree.innerText = parseInt(visafree.innerText)*parseInt(peopleNumber.innerText)+parseInt(ktfree.innerText)
        }
    }
}
/*拒签险选中*/
function checkfee(j){
    if(j.checked) {
        antiFee.style.display = "block";
        var valanti= parseInt(j.parentNode.previousSibling.previousSibling.childNodes[1].innerText);
        antifree.innerText = valanti
        if (checks[numSum].checked) {
            sumFree.innerText = (parseInt(visafree.innerText) * (parseInt(peopleNumber.innerText))) + parseInt(free.innerText) * (parseInt(peopleFree.innerText)) + parseInt(antifree.innerText) * (parseInt(peopleanti.innerText)) + parseInt(ktfree.innerText)
        }else{
            sumFree.innerText = (parseInt(visafree.innerText) * (parseInt(peopleNumber.innerText)))+ parseInt(antifree.innerText) * (parseInt(peopleanti.innerText)) + parseInt(ktfree.innerText)
        }
    }else{
        checkBoxfee.checked = false;
        antiFee.style.display="none";
        if(checks[numSum].checked){
            sumFree.innerText=(parseInt(visafree.innerText)*(parseInt(peopleNumber.innerText)))+parseInt(free.innerText)*(parseInt(peopleFree.innerText))+parseInt(ktfree.innerText)
        }else{
            sumFree.innerText=(parseInt(visafree.innerText)*(parseInt(peopleNumber.innerText)))+parseInt(ktfree.innerText)
...
var provinceList = [
    {name:'请选择省份',cityList:[{name:'请选择市',areaList:['请选择地区']}]},
    {name:'北京市', cityList:[
        {name:'市辖区',
...
function orderContact() {
    var uname = document.getElementsByClassName("username")[0].innerText/*账号*/
    var orderVisaName = document.getElementById("orderVisaName").innerText
    var contactUname = document.getElementsByName("contactUname")[0].value;
    var contactTel = document.getElementsByName("contactTel")[0].value
    var contactEmail = document.getElementsByName("contactEmail")[0].value
    var goDate = document.getElementsByName("goDate")[0].value

    var cmbProvince = $("#cmbProvince option:selected").text();
    var cmbCity = $("#cmbCity option:selected").text();
    var cmbArea = $("#cmbArea option:selected").text();
    var address = document.getElementsByName("address")[0].value;

    var countPeople=document.getElementById('peopleNumber').innerText
    var ktfree=document.getElementById('ktfree').innerText

    var insurancxeName = document.getElementById("insurancxeName").innerText;
    var free = document.getElementById("free").innerText;

    var refusalName = document.getElementById("refusalName").innerText;
    var antifree = document.getElementById("antifree").innerText;

    var sumFree = document.getElementById("sumFree").innerText
    var userid;
    if(uname!=""&&uname!="登录"){

        ajax('get','/queryuserid.do','uname='+uname,function () {
        ...
        var datasum = year+month+day+hour+minutes+seconds;
        var mathDate=Math.round(Math.random()*900+100);
        var matchNum='V'+userid+mathDate+datasum;
        var orderData = year+'-'+month+'-'+day;

        ajax("post","/orderContact.do","uname="+uname+"&matchNum="+matchNum+"&orderVisaName="+orderVisaName+
            "&contactUname="+contactUname+"&contactTel="+contactTel+"&contactEmail="+contactEmail+
            "&goDate="+goDate+"&cmbProvince="+cmbProvince+"&cmbCity="+cmbCity+"&cmbArea="+cmbArea+
            "&address="+address+"&ktfree="+ktfree+"&free="+free+"&insurancxeName="+insurancxeName+
            "&refusalName="+refusalName+"&antifree="+antifree+"&sumFree="+sumFree+"&countPeople="+countPeople+"&orderData="+orderData,
            function () {
                console.log(matchNum)
                orderPeople();
                window.location.href='/xr_payfor.html?matchNum='+matchNum+"&sumFree="+sumFree;
            })
    }else{
        localStorage.cName=contactUname;
        var gethref=localStorage.getItem("cName");
        contactUname=gethref
        console.log("contactUname",contactUname)
        sessionStorage.myhref=location.href;
...
function orderPeople() {
    var pelNameArr=[];//出游人姓名
    var typeNameArr=[];//出游人客户类型
    var birthDateArr=[];//出游人出生日期

    var pelName = $(".orderInput1 input[name='pelName']")
    var typeName =$('.typeSelect option:selected');
    // var birthDate = $(".orderInput1 input[name='birthDate']")
    for(var i=0;i<pelName.length;i++){
        pelNameArr.push(pelName.eq(i).val());
    }
    for(var i=0;i<typeName.length;i++){
        typeNameArr.push(typeName.eq(i).text());
    }
    // for(var i=0;i<birthDate.length;i++){
    //     birthDateArr.push(birthDate.eq(i).val());
    // }
    $.ajax({
        type:"POST",//类型
        url:"/orderPeople.do",//地址
        data:{//参数
            pelNameContro:pelNameArr,
            typeNameContro:typeNameArr,
            // birthDateContro:birthDateArr,
            contactUname:$("[name='contactUname']").val(),
        },
        traditional:true,//传统的方式来序列化数据,传数组是必须设置为true
        async:true,
