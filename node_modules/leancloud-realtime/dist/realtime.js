  ...
  Cache.prototype.get = function get(key) {
    ...
    if (cache) {
      ...
      if (value !== Expirable.EXPIRED) {
        debug$1('[%s] hit: %s %O', this.name, key, value);
        return value;
      }
...
var Realtime = function (_EventEmitter) {
  ...
  Realtime._fetchRTMServers = function _fetchRTMServers(_ref9) {
    ...
    return this._getServerUrls({ appId: appId, region: region, server: server }).then(tap(debug$4)).then(function (_ref10) {
...
var beforeCommandDispatch = function beforeCommandDispatch(command, realtime) {
  ...
  if (targetClient) {
    ...
    debug$11('[WARN] Unexpected message received without any live client match: %O', trim(command));
  }
  return false;
};
